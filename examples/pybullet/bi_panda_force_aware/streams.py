...
from examples.pybullet.utils.pybullet_tools.utils import pairwise_collision, get_distance, multiply, set_pose, \
    interpolate_poses, invert, wait_if_gui, is_b1_on_b2, get_mass, get_pose, get_COM
...
def get_cfree_traj_grasp_pose_test(problem, collisions=True):
    def test(c, a, b1, g, b2, p2):
        print("in free traj grasp pose test")
...
def get_forces_balanced_test(problem):
    def test(c, b1, p1, b2):
        state = c.assign()
        bodiesOnB2 = []
        comR = get_COM()
