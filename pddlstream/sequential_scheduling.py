...
def sequential_stream_plan(evaluations, goal_expression, domain, stream_results, unit_costs=True, **kwargs):
    ...
    result_from_head = defaultdict(list)
    for evaluation, stream_result in opt_evaluations.items():
        #result_from_head[evaluation.head].append((evaluation.value, stream_result))
        result_from_head[evaluation.head].append(stream_result)
    ...
    for i, (name, args) in enumerate(action_plan):
        ...
        #cost = pddl.Increase(fluent=fluent, expression=expression)
        cost = None
        ...
        if (action.cost is not None) and isinstance(action.cost.expression, pddl.PrimitiveNumericExpression):
            func = action.cost.expression.symbol
            args = tuple(obj_from_pddl(var_mapping[p]) for p in action.cost.expression.args)
            head = Head(func, args)
            [stream_result] = result_from_head[head]
            if stream_result is not None:
                function_plan.append(stream_result)
