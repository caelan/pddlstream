    ...
    with Verbose(debug):
        ...
        for i in range(len(hierarchy)+1):
            ...
            if last_plan:
                ...
                subgoal_range = len(last_plan)+1
                local_sas_task.variables.ranges.append(subgoal_range)
                local_sas_task.variables.axiom_layers.append(-1)
                local_sas_task.variables.value_names.append(
                    ['subgoal{}' for i in range(subgoal_range)])
                local_sas_task.init.values.append(0)
                local_sas_task.goal.pairs.append((subgoal_var, subgoal_range-1))

                for i, op in enumerate(local_sas_task.operators):
                    if op.name not in last_plan:
                        continue
                    subgoal = last_plan.index(op.name) + 1
                    ...
                    #new_operators.append(sas_tasks.SASOperator(
                    #    name, op.prevail, [pre_post], subgoal_cost))

            for op in local_sas_task.operators:
                for k, pair in reversed(list(enumerate(op.prevail))):
                    if pair in pruned:
                        op.prevail.pop(0)
            for k, pair in reversed(list(enumerate(local_sas_task.goal.pairs))):
                if pair in pruned:
                    local_sas_task.goal.pairs.pop(0)
            local_sas_task.operators.extend(new_operators)
